...
const contentTypesArray = ["lesson", "worksheet", "assignment", "test", "quiz", "notes", "reading_material", "other"]; // Keep in sync
...
exports.getWeightsForChildSubject = async (req, res) => {
    ...
    try {
        ...
        const fullWeights = contentTypesArray.map(ct => ({
            content_type: ct,
            weight: weightsMap.has(ct) ? parseFloat(weightsMap.get(ct)) : (["test", "quiz", "assignment", "worksheet"].includes(ct) ? 0.10 : 0.00) // Example default
        }));
